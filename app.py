...
def editar(id):
    # Buscamos al pasante por su ID. Si no existe, da error 404.
    p = pasante.query.get_or_404(id) 
    
    if request.method == 'POST':
        # Aquí se guardan los cambios cuando presionas "Actualizar"
        ...
        db.session.commit()
        flash(f'¡Datos de {p.nombre} actualizados!', 'success')
        return redirect(url_for('index'))
    
    # IMPORTANTE: Enviamos el objeto 'p' para que el HTML lo use
